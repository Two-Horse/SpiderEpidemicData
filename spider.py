...
def get_tencent_data():
    ...
    #历史数据
    r_last = requests.get(url_last,headers)
    res_last = json.loads(r_last.text)
...
def get_baidu_hot():
    ...
    browser = Chrome(options=option,executable_path=r"D:\Jupyter note\chromedriver.exe")
    browser.get(url)
    # 找到展开按钮
    dl = browser.find_element_by_css_selector('#ptab-1 > div.Virus_1-1-316_2SKAfr > div.Common_1-1-316_3lDRV2')
    dl.click()
    time.sleep(1)
    # 找到热搜标签
